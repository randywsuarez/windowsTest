		...
		<div v-else>
			<div class="keyboard-wrapper">
				...
				<div
					v-if="isCompact"
					id="compact-keyboard"
					class="keyboard-container compact-keyboard-container"
				>
					<div
						v-for="(row, rowIndex) in compactKeyboardData.rows"
						:key="'compact-row-' + rowIndex"
						class="row compact-row"
					>
				...
				<div v-else id="complete-keyboard" class="keyboard-container">
					<div
						v-for="(row, rowIndex) in completeKeyboardData.rows"
						:key="'complete-row-' + rowIndex"
						class="row complete-row"
					>
						<div
							...
							:id="key.id"
							:class="{
								'numpad-enter': key.id === 'kb_btn_NumpadEnter',
								blank: key.class && key.class.includes('blank'),
				...
				<div id="kb_box_b" class="row justify-center q-pt-md q-mt-md q-gutter-md">
					...
					<q-btn
						...
						:disable="!allKeysPressed && !isBypassEnabled"
						class="col-3"
						tabindex="-1"
					/>
				</div>
...
<script>
	...
	/* import { getCurrentEnvironment } from '../utils/envHelper'; */
	...
	const preventToggleSpaceActivation = (event) => {
		if (event.code === 'Space' || event.keyCode === 32) {
			...
			if (
				toggleElement &&
				(document.activeElement === toggleElement || document.activeElement.closest('.q-toggle'))
			) {
	...
	export default {
		data() {
			return {
				...
				isBypassEnabled: false, // Variable para hacer bypass de la validación
			}
		},
		methods: {
			...
			toggleKeyboard() {
				...
				// Verificar si todas las teclas han sido presionadas después de cambiar el teclado
				this.$nextTick(() => {
			...
			checkAllKeysPressed() {
			    ...
			    // Si todas las teclas han sido presionadas, mostrar una notificación
			    if (allPressed && !this.allKeysPressed) {
			        this.$q.notify({
			            type: 'positive',
			            message: 'All keys have been tested successfully!',
			            position: 'top',
			            timeout: 3000,
			        })
			    }
					...
					row.keys.forEach((key) => {
						if (key.type === 'split-column') {
							...
							key.keys.forEach((splitKey) => {
								if (
									splitKey.id &&
									!keyIds.includes(splitKey.id) &&
									splitKey.id.indexOf('kb_btn_Blank') === -1
								) {
			...
			captureKeyboard(status) {
				...
				if (element) {
					html2canvas(element).then((canvas) => {
						...
						// Detener la escucha de eventos de teclado
			...
			// Nuevo método para detener todos los event listeners de teclado
			stopKeyboardListeners() {
				// Eliminar los event listeners de teclado
				...
				// Restaurar el comportamiento predeterminado de las teclas
			...
			handleKeydown(event) {
				...
				// Combinación secreta para habilitar/deshabilitar el bypass: Ctrl+Alt+B
				if (event.ctrlKey && event.altKey && event.code === 'KeyB') {
					this.isBypassEnabled = !this.isBypassEnabled
					this.$q.notify({
						type: this.isBypassEnabled ? 'warning' : 'info',
						message: this.isBypassEnabled ? 'Bypass mode enabled' : 'Bypass mode disabled',
						position: 'top',
						timeout: 3000,
					})
					return
				...
				// Manejo especial para Print Screen
				if (event.keyCode === 44 || event.code === 'PrintScreen') {
					this.pressKey('kb_btn_PrintScreen')
					return
				}
			...
			handleWrongKey(keyId, code) {
				...
				const otherKeyboardData = this.isCompact
					? this.completeKeyboardData
					: this.compactKeyboardData
			...
			handleKeyup(event) {
				...
				// Código 44 corresponde a PrintScreen en algunos navegadores
				if (event.keyCode === 44 || event.code === 'PrintScreen') {
					this.pressKey('kb_btn_PrintScreen')
				}
			},
		...
		mounted() {
			...
			/* // Verificar si estamos en un entorno diferente a producción
			const currentEnvironment = getCurrentEnvironment();
			if (currentEnvironment !== 'production') {
				this.isBypassEnabled = true; // Habilitar bypass en entornos que no sean producción
			} */
			
			// Añadir un log para depuración cuando se monte el componente
			console.log('Keyboard component mounted, bypass enabled:', this.isBypassEnabled)
			...
			// Agregar un manejador específico para la tecla Print Screen
			...
			// Eliminar la línea que habilita el bypass por defecto
			// this.isBypassEnabled = true
			// console.log('Bypass enabled for testing')
		},
...
<style scoped>
	...
	.keyboard-container {
		...
		transform-origin: top center; /* Asegurar que la escala se aplique desde el centro superior */
		margin-bottom: 20px; /* Espacio para evitar solapamiento al escalar */
		transition: transform 0.3s ease; /* Transición suave al cambiar la escala */
	}
