          ...
          if (
            ...
            if (this.recordarCredenciales) {
              ...
              await this.$rsNeDB("credenciales").insert({
                ...
                authToken: respuesta.AuthToken,
                id: respuesta.Id,
                tenant: respuesta.tenant,
              });
            ...
            console.error(
              "Inicio de sesión fallido. Verifica usuario y contraseña."
            );
          }
      ...
      return new Promise((resolve, reject) => {
        ...
        for (let s of this.$env.project) {
          ...
          fetch(`${s.url}/Login/Authenticate`, options)
            ...
            .then((response) => resolve(response)) // Resolver la promesa con la respuesta
...
.login-card {
  background: rgba(
    ...
    255,
    0.1
  ); /* Color blanco transparente para el glassmorphism */
